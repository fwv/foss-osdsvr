// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.28.0
// 	protoc        v3.15.8
// source: internal/pkg/core/init.proto

package core

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

type OSDServerInit struct {
	state         protoimpl.MessageState
	sizeCache     protoimpl.SizeCache
	unknownFields protoimpl.UnknownFields

	ID int64 `protobuf:"varint,1,opt,name=ID,proto3" json:"ID,omitempty"`
}

func (x *OSDServerInit) Reset() {
	*x = OSDServerInit{}
	if protoimpl.UnsafeEnabled {
		mi := &file_internal_pkg_core_init_proto_msgTypes[0]
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		ms.StoreMessageInfo(mi)
	}
}

func (x *OSDServerInit) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OSDServerInit) ProtoMessage() {}

func (x *OSDServerInit) ProtoReflect() protoreflect.Message {
	mi := &file_internal_pkg_core_init_proto_msgTypes[0]
	if protoimpl.UnsafeEnabled && x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OSDServerInit.ProtoReflect.Descriptor instead.
func (*OSDServerInit) Descriptor() ([]byte, []int) {
	return file_internal_pkg_core_init_proto_rawDescGZIP(), []int{0}
}

func (x *OSDServerInit) GetID() int64 {
	if x != nil {
		return x.ID
	}
	return 0
}

var File_internal_pkg_core_init_proto protoreflect.FileDescriptor

var file_internal_pkg_core_init_proto_rawDesc = []byte{
	0x0a, 0x1c, 0x69, 0x6e, 0x74, 0x65, 0x72, 0x6e, 0x61, 0x6c, 0x2f, 0x70, 0x6b, 0x67, 0x2f, 0x63,
	0x6f, 0x72, 0x65, 0x2f, 0x69, 0x6e, 0x69, 0x74, 0x2e, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x12, 0x04,
	0x63, 0x6f, 0x72, 0x65, 0x22, 0x1f, 0x0a, 0x0d, 0x4f, 0x53, 0x44, 0x53, 0x65, 0x72, 0x76, 0x65,
	0x72, 0x49, 0x6e, 0x69, 0x74, 0x12, 0x0e, 0x0a, 0x02, 0x49, 0x44, 0x18, 0x01, 0x20, 0x01, 0x28,
	0x03, 0x52, 0x02, 0x49, 0x44, 0x42, 0x1a, 0x5a, 0x18, 0x6f, 0x73, 0x64, 0x73, 0x76, 0x72, 0x2f,
	0x69, 0x6e, 0x74, 0x65, 0x72, 0x6e, 0x61, 0x6c, 0x2f, 0x70, 0x6b, 0x67, 0x2f, 0x63, 0x6f, 0x72,
	0x65, 0x62, 0x06, 0x70, 0x72, 0x6f, 0x74, 0x6f, 0x33,
}

var (
	file_internal_pkg_core_init_proto_rawDescOnce sync.Once
	file_internal_pkg_core_init_proto_rawDescData = file_internal_pkg_core_init_proto_rawDesc
)

func file_internal_pkg_core_init_proto_rawDescGZIP() []byte {
	file_internal_pkg_core_init_proto_rawDescOnce.Do(func() {
		file_internal_pkg_core_init_proto_rawDescData = protoimpl.X.CompressGZIP(file_internal_pkg_core_init_proto_rawDescData)
	})
	return file_internal_pkg_core_init_proto_rawDescData
}

var file_internal_pkg_core_init_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_internal_pkg_core_init_proto_goTypes = []interface{}{
	(*OSDServerInit)(nil), // 0: core.OSDServerInit
}
var file_internal_pkg_core_init_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_internal_pkg_core_init_proto_init() }
func file_internal_pkg_core_init_proto_init() {
	if File_internal_pkg_core_init_proto != nil {
		return
	}
	if !protoimpl.UnsafeEnabled {
		file_internal_pkg_core_init_proto_msgTypes[0].Exporter = func(v interface{}, i int) interface{} {
			switch v := v.(*OSDServerInit); i {
			case 0:
				return &v.state
			case 1:
				return &v.sizeCache
			case 2:
				return &v.unknownFields
			default:
				return nil
			}
		}
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: file_internal_pkg_core_init_proto_rawDesc,
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_internal_pkg_core_init_proto_goTypes,
		DependencyIndexes: file_internal_pkg_core_init_proto_depIdxs,
		MessageInfos:      file_internal_pkg_core_init_proto_msgTypes,
	}.Build()
	File_internal_pkg_core_init_proto = out.File
	file_internal_pkg_core_init_proto_rawDesc = nil
	file_internal_pkg_core_init_proto_goTypes = nil
	file_internal_pkg_core_init_proto_depIdxs = nil
}
